...
def index():
    ...
    return render_template('index.html', title="ТЫРЫПЫРЫ",
                           started_ids=product_search.started_ids,
                           href_id=product_search.href_id,
...
def file_id(id):
    ...
    return render_template('ids.html', title="ТЫРЫПЫРЫ",
                           started_ids=product_search.started_ids,
                           href_id=product_search.href_id,
                           href_root=product_search.href_root,
                           identificator=id,
                           started_products=product_search.started_products,
                           change_color=product_search.change_color,
                           start_quantity=product_search.start_quantity,
                           produced_quantity=product_search.produced_quantity,
                           reject_quantity=product_search.reject_quantity,
                           amount1=amount1, search4=product_search.search4,
                           **context)
...
def file_id2(id):
    ...
    return render_template('ids2.html', title="ТЫРЫПЫРЫ",
                           started_ids=product_search.started_ids,
                           href_id=product_search.href_id,
                           href_root=product_search.href_root,
                           identificator=id, **context)
...
def file_id3(id):
    ...
    return render_template('ids3.html', title="ТЫРЫПЫРЫ",
                           started_ids=product_search.started_ids,
                           href_id=product_search.href_id,
                           href_root=product_search.href_root,
                           identificator=id, **context)
...
@app.route("/1")
def first():
    return render_template('1.html', title="OMG", values_count1=values_count1,
                           amount1=amount1, search1=product_search.search1,
                           search4=product_search.search4,
                           change_color=product_search.change_color,
                           produced_quantity=product_search.produced_quantity,
                           reject_quantity=product_search.reject_quantity,
                           file_search=product_search.file_search)
# страница с выводом состава файла второго типа


@app.route("/2")
def second():
    return render_template('2.html', title="MIN_DESIGN",
                           values_count2=values_count2, amount2=amount2,
                           search2=product_search.search2)
# страница с выводом состава файла третьего типа


@app.route("/3")
def third():
    return render_template('3.html', title="OPT_LOCAL_MIFARE", data=data,
                           amount3=amount3, search3=product_search.search3)
...
def submit1():
    ...
    change = db_session.query(ProductInFile).filter_by(
        id=request.form.get('id'),
        input_file_id=request.form.get('file_id')).first()
...
def submit2():
    ...
    if db_session.query(ProductInFile).filter_by(
            input_file_id=request.form.get('file_id'),
            product_in_file_status="started").count() > 0:
        return "Необходимо принять каждую строку"
    else:
        product_in_file = product_in_file.query.filter_by(
            input_file_id=request.form.get('file_id'),
            product_in_file_status="processed").all()
        ...
        for product in product_in_file:
            ...
            produce = Storage(
                ...
                product_quantity=product.produced_quantity)
            db_session.add(produce)
            reject = Storage(
                product_id=product.product_id,
                product_type="perso_reject",
                product_quantity=product.reject_quantity)
