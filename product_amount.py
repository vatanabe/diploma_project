...
"""def amount1(code):
    ...
    for (id, prod_chip, prod_vendor) in db_session.query(
            Product.id, Product.prod_chip, Product.prod_vendor).filter_by(
            product_code=code):
        for (product_quantity,) in db_session.query(
                Storage.product_quantity).filter_by(
                product_type='new', product_id=id,
...
def amount1(product_name):
    ...
    for (id, prod_chip, prod_vendor) in db_session.query(
        ...
        for (product_quantity,) in db_session.query(
                Storage.product_quantity).filter_by(
                product_type='new', product_id=id, chip=prod_chip,
                vendor=prod_vendor):
            total_product_quantity += product_quantity
        for (used_product_quantity,) in db_session.query(
                Storage.product_quantity).filter_by(
                product_type='complete', product_id=id,
                chip=prod_chip, vendor=prod_vendor):
...
def amount2(code):
    for (id, prod_chip, prod_vendor) in db_session.query(
        ...
        for (product_quantity,) in db_session.query(
                    Storage.product_quantity).filter_by(
                    product_type='colored', product_id=id, chip=prod_chip,
                    vendor=prod_vendor):
...
def amount3(code):
    for (id,) in db_session.query(Product.id).filter_by(city_code=code):
        for (product_quantity,) in db_session.query(
                Storage.product_quantity).filter_by(
                product_type='colored', product_id=id):
            return product_quantity
    return 0
