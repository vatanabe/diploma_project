...
def search1(code):
    for (product_name,) in db_session.query(Product.product_name).filter_by(
            product_code=code, used=1):  # , special_design=0):
        return product_name
...
def search3(code):
    for (product_name,) in db_session.query(Product.product_name).filter_by(
            city_code=code):
        return product_name
# поиск id в таблице текущих работ по коду продукта
# (необходимо дополнить поиск по id файла в работе)


def search4(product_name, file_id):
    for (id,) in db_session.query(Product.id).filter_by(
            product_name=product_name):
        for (id,) in db_session.query(ProductInFile.id).filter_by(
                product_id=id, input_file_id=file_id):
            return id
# поиск значения статуса продукта в текущих работах для изменения цвета строки
# (подтверждение) (необходимо дополнить поиск по id файла в работе)


def change_color(product_name, file_id):
    for (id,) in db_session.query(Product.id).filter_by(
            product_name=product_name):
        for (product_in_file_status,) in db_session.query(
                ProductInFile.product_in_file_status).filter_by(
                product_id=id, input_file_id=file_id):
...
def start_quantity(product_name, file_id):
    for (id,) in db_session.query(Product.id).filter_by(
            product_name=product_name):
        for (input_quantity,) in db_session.query(
                ProductInFile.input_quantity).filter_by(
                product_id=id, input_file_id=file_id):
            return input_quantity
# поиск количества выполненнго продукта в текущих работах
# (необходимо дополнить поиск по id файла в работе)
...
# поиск количества бракованного продукта в текущих работах
# (необходимо дополнить поиск по id файла в работе)
...
def reject_quantity(product_name, file_id):
    for (id,) in db_session.query(Product.id).filter_by(
            product_name=product_name):
        for (reject_quantity,) in db_session.query(
                ProductInFile.reject_quantity).filter_by(
                product_id=id, input_file_id=file_id):
            return reject_quantity
# функция поиска названия файла в папке для отображения его
# на странице текущей работы (бесполезная??)
...
def started_files():
    filenames = []
    for (input_file_name,) in db_session.query(
            InputFile.input_file_name).filter_by(file_status='started'):
...
def started_ids(filename):
    for (id,) in db_session.query(InputFile.id).filter_by(
...
def started_products(file_id):
    started_products = []
    for (product_id,) in db_session.query(ProductInFile.product_id).filter_by(
            input_file_id=file_id):
        for (product_name,) in db_session.query(
                Product.product_name).filter_by(id=product_id):
